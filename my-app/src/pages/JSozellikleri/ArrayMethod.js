...
function UrunListesi() {
  ...
  const findUrun = urunler.find((urun) => urun.id === 3);
  ...
  const someUrun = urunler.some((urun) => urun.fiyat < 2); // false, dizide bir tane bile fiyatı 2'den düşük olan yok
  const someUrun2 = urunler.some((urun) => urun.fiyat > 9); // true, dizide en az bir tane fiyatı 9'dan büyük olan var
  ...
  const everyUrun = urunler.every((urun) => urun.fiyat > 1); // true, tüm ürün fiyatları 1'den büyük
  const everyUrun2 = urunler.every((urun) => urun.fiyat < 10); // false, tüm ürün fiyatları 10'dan küçük DEĞİL
  ...
  const includesUrun = urunler.some((urun) => urun.name.includes("Kalem")); // true, bazı ürün isimleri Kalem içeriyor
  const includesUrun2 = urunler.every((urun) => urun.name.includes("Kalem")); // false, tüm ürün isimleri Kalem içermiyor
  ...
  const kalemUrunler = urunler
    .filter((urun) => urun.name.includes("Kalem"))
    .map((urun) => `${urun.name} fiyatı ${urun.fiyat} liradır.`);

  urunler
    .filter((urun) => urun.name.includes("Kalem")) // Önce Kalem içeren ürünleri bul
    .map((urun) => `${urun.name} fiyatı ${urun.fiyat} liradır.`); // Ardından bulunan elemanlara verilen callbak fonksiyonu
...
*/
  ...
  const toplamFiyat = urunler.reduce((oncekiDeger, suankiUrun) => {
    return oncekiDeger + suankiUrun.fiyat;
  }, 0);

  // Toplam fiyat hesaplama
  const toplamFiyat1 = urunler.reduce((toplam, urun) => toplam + urun.fiyat, 0);

  // Ürün isimlerinin tek satırda yazdırılması
  const urunIsimleri = urunler.reduce(
    (isimler, urun) => isimler + " " + urun.name,
    "Ürün İsimleri:"
  );
  console.log(urunIsimleri); // "Ürün İsimleri: Kalem Defter Silgi Kalemtraş"

  // Yeni bir dizi oluşturma
  const yeniUrunListesi = urunler.reduce(
    (yeniUrunListesi, urun) => [
      ...yeniUrunListesi,
      `${urun.name} ${urun.fiyat} liradır.`,
    ],
  ...
  console.log(yeniUrunListesi); // ["Kalem 5 liradır.", "Defter 10 liradır.", "Silgi 2 liradır.", "Kalemtraş 7 liradır."]

  return (
    <div className="container">
      <h1 className="renk">Find methodu </h1>
      <h2>Aranan Ürün: {findUrun.name}</h2>
      <h2>
        <p>Fiyatı: {findUrun.fiyat} TL</p>
      </h2>
      <br></br>
      {/* ------------------------------------------------------------- */}

      <h1 className="renk">Some methodu </h1>
      <h2>Yüksek Fiyatlı Ürün Var Mı? {someUrun ? "Evet" : "Hayır"}</h2>
      <h2>Yüksek Fiyatlı Ürün Var Mı? {someUrun2 ? "Evet" : "Hayır"}</h2>
      <br></br>
      {/* ------------------------------------------------------------- */}

      <h1 className="renk">Every methodu </h1>

      <h2>Düşük Fiyatlı Ürünler Mi? {everyUrun ? "Evet" : "Hayır"}</h2>
      <h2>Düşük Fiyatlı Ürünler Mi? {everyUrun2 ? "Evet" : "Hayır"}</h2>
      <br></br>
      {/* ------------------------------------------------------------- */}

      <h1 className="renk">includes methodu </h1>

      <h2>Kalem Var Mı? {includesUrun ? "Evet" : "Hayır"}</h2>
      <h2>Kalem Var Mı? {includesUrun2 ? "Evet" : "Hayır"}</h2>
      <br></br>
      {/* ------------------------------------------------------------- */}

      <h1 className="renk">Map methodu </h1>

      {urunler.map((urun) => (
        <h1>
          <li key={urun.id}>{` ${urun.name} fiyatı ${urun.fiyat} liradır.`}</li>
        </h1>
      ))}
      <br></br>
      {/* ------------------------------------------------------------- */}

      <h1 className="renk">filter methodu </h1>

      <h1>{kalemUrunler} </h1>
      <br></br>
      {/* ------------------------------------------------------------- */}

      <h1 className="renk">Reduce methodu </h1>

      <h1>Toplam Fiyat: {toplamFiyat} lira</h1>
      <h1>Toplam Fiyat: {toplamFiyat1} lira</h1>
      <h1>urun Isimleri: {urunIsimleri} lira</h1>
      <h1>yeni Urun Listesi: {yeniUrunListesi} lira</h1>
    </div>
  );
}
